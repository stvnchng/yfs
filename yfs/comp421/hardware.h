 ...
 *  the current state of a running process.  ContextSwitch()
...
/*
 *  Switching process context (which is done from inside the kernel) is
 ...
 *
 *  To make the job of switching process context easier in the project,
 ...
 *  kernel stack, saves the current hardware context into the SavedContext
 ...
 *  to MySwitch.  You should use them (for example) to point to the
 *  current process's PCB and to the PCB of the new process to be
 *  context switched in, respectively.
 *
 *  In MySwitch, you should do whatever you need to do to context
 *  switch between these two processes.  The "SavedContext *" value you
 *  return from your MySwitch function should point to a SavedContext
 *  containing the context of the new process to run.  This SavedContext
 *  *must* have been created by some earlier call to ContextSwitch.
 *  If you return the same "SavedContext *" pointer that was passed
 *  to your MySwitch function on this call, then no change actually
 *  occurs, but the SavedContext will still have been saved in memory
 *  by this call to ContextSwitch, which you can then use later use
 *  on some other ContextSwitch call.
 *
 *  You should use ContextSwitch to in effect "block" a process
 ...
 *  this process is later context switched back to, ContextSwitch
